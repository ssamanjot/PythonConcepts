...
def main(NoOfProcess,Urls):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=NoOfProcess) as executor:
...
def visualize_processing_time(results,colors,MaxWorker):
    ...
    plt.ylabel("Tasks")
    plt.xlabel("Seconds")
    plt.title("Tasks for URL Server Responses; " + str(MaxWorker) +" Process")
...
with open('C:/Users/cool dude/PycharmProjects/hello\Multiprocessing/Data/url.txt', "r", encoding=None) as y:
    for line in y:
        Urls.append(line)
